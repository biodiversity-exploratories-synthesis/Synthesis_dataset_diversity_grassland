...
#correct dcast.data table with fill=NA?
...
setnames(bird,names(bird),c("Plot_bexis","Species","value","Fun_group_fine")) #rename to match main table
...
grl2<-rbindlist(list(grl2,bird),use.names=TRUE)

rm(allbirdtr,birdtr,bird,newsp); gc()
...
grl2[Trophic_level=="protist.plant.parasite",Trophic_level:="plantparasite.protist"]
grl2[Trophic_level=="protist.parasite.nonplant",Trophic_level:="parasitenonplant.protist"]
...
#all good: save
fwrite(grl,"Exploratories/Data/GRASSLANDS/201111_EP_species_diversity_GRL.txt",row.names=F,quote=F,sep=";",na=NA)
fwrite(tr,"Exploratories/Data/GRASSLANDS/201111_EP_species_info_GRL.txt",row.names=F,quote=F,sep=";",na=NA)
...
#birds 2018 were also sampled at larger spatial scales 50 and 100 (I used the EP level for compatibility with previous datasets)
