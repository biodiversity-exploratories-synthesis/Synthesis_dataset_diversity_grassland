...
#17.add arthropod temporal dataset.. to be done
...
tt<-merge(tt[,.(Species,Plot,value)],
          unique(tt[,!c("Plot","value"),with=F],by="Species"),
          by="Species",all.x=T)
tt<-tt[is.na(value)]
...
tt<-setDT(tt)[CJ(Species=Species,Plot=Plot,unique=T), on=.(Species, Plot)]
tt<-merge(tt[,.(Species,Plot,value)],
          unique(tt2[,!c("Plot","value"),with=F],by="Species"),
          by="Species",all.x=T)
tt<-tt[is.na(value)]
grl2<-rbindlist(list(grl2,tt),use.names = T)
rm(tt,tt2,i)
...
#all good: save
fwrite(grl,"N:/Exploratories/Data/GRASSLANDS/190802_EP_species_diversity_GRL.txt",row.names=F,quote=F,sep=";",na=NA)
fwrite(tr,"N:/Exploratories/Data/GRASSLANDS/190802_EP_species_info_GRL.txt",row.names=F,quote=F,sep=";",na=NA)
